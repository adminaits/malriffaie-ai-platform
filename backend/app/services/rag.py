...
def _latest_ai_settings() -> dict:
    res = supabase.table("ai_settings").select("*").order("created_at", desc=True).limit(1).execute()
...
def retrieve_context(query: str, limit: int = 8) -> dict:
    # Simple Postgres text search fallback. For production, add pgvector embeddings and RPC match_documents().
    kb = supabase.table("knowledge_base").select("id,source_type,source_id,content,metadata").ilike("content", f"%{query[:80]}%").limit(limit).execute().data or []
    products = supabase.table("products").select("*").eq("available", True).order("created_at", desc=True).limit(30).execute().data or []
    services = supabase.table("services").select("*").eq("available", True).order("created_at", desc=True).limit(30).execute().data or []
    return {"knowledge": kb, "products": products, "services": services}
...
def recommend_products(message: str, products: list[dict]) -> list[dict]:
    ...
    for p in products:
        ...
        if score or any(k in low for k in ["marketing", "agreement", "hr", "feasibility", "consultation", "retainer"]):
...
def _wants_product_list(message: str) -> bool:
    ...
    return any(phrase in low for phrase in [
        "list all products", "all products", "show products", "products list", "what products", "available products"
    ])
...
def _product_list_answer(products: list[dict]) -> str:
    ...
    for idx, product in enumerate(products, 1):
        lines.append(f"{idx}. {product.get('name')} — {_format_price(product.get('price'), product.get('currency'))}")
    lines.append("")
    lines.append("You can click a product in the sidebar to view details, or tell me what you need and I will recommend the best option.")
    return "\n".join(lines)
...
async def answer_chat(message: str, visitor_id: str | None = None, lang: str = "en", ip_hash: str | None = None) -> dict:
    ...
    if _wants_product_list(message):
        ...
        if product and any(k in message.lower() for k in ["tell", "detail", "price", "buy", "about", "more"]):
            answer = _product_detail_answer(product)
            recommended = [product]

    if answer is None:
        prompt_template = cfg.get("system_prompt") or DEFAULT_PROMPT
        prompt = prompt_template.format(site="Malriffaie", site_url="", url="", lang=lang, date=date.today().isoformat())
        prompt += "\n\nProducts:\n" + "\n".join([f"- {p.get('name')} | {_format_price(p.get('price'), p.get('currency'))} | {p.get('description','')}" for p in ctx["products"]])
        prompt += "\n\nServices:\n" + "\n".join([f"- {s.get('name')} | {_format_price(s.get('price'), s.get('currency'))} | {s.get('description','')}" for s in ctx["services"]])
        prompt += "\n\nKnowledge context:\n" + "\n---\n".join([k.get("content", "")[:1200] for k in ctx["knowledge"]])
        prompt += f"\n\nCustomer message: {message}\nAnswer:"

        model_name = cfg.get("custom_model_name") if cfg.get("model_name") == "custom" else cfg.get("model_name")
        hf = HuggingFaceClient(
            token=cfg.get("hugging_face_token"),
            model=model_name or settings.default_hf_model,
            endpoint_url=cfg.get("custom_endpoint_url"),
        )
        ...
        if not answer or "AI is not configured" in answer:
            answer = cfg.get("fallback_message") or "I do not have that information yet. I can arrange a human handoff for you."
    ...
    return {"answer": answer, "products": recommended, "sources": ctx["knowledge"]}
