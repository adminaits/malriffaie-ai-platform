...
class HuggingFaceClient:
    def __init__(
        ...
        self.token = token
        self.model = (model or settings.default_hf_model or "HuggingFaceH4/zephyr-7b-beta").strip()

        clean_endpoint = (endpoint_url or "").strip()
        ...
        *,
        ...
        try:
            ...
            if res.status_code >= 400:
                return f"AI connection error {res.status_code}: {res.text[:500]}"
            ...
            return f"AI connection exception: {str(exc)}"
...
async def test_hf_connection(
    ...
    output = await client.generate(
        "Reply with: connection ok",
        max_tokens=20,
        timeout=45,
    )

    ok = "connection ok" in output.lower() or "error" not in output.lower()
